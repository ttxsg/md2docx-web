...
【代码块内的内容要求】
你在代码块内输出一份 Pandoc 友好的 Markdown 文档（用于转 Word/docx），并遵守：
...
3) 公式用 LaTeX：
   ...
   - 独立：
     ...
     $$
   - 不要使用 \\[ \\] 形式
4) 表格用标准 Markdown 表格（可选）
5) 列表用 - 或 1.，子级缩进用 2 或 4 个空格
6) 图片用 ![caption](path_or_url)（可选）
7) 引用用纯文本 [1] [2]（不要 BibTeX）
8) 不要使用 HTML 标签（如 <br> <span>）
9) 尽量使用英文半角符号

现在请根据我提供的内容生成最终文档（我将粘贴原始内容/提纲/段落）：
（我将粘贴原始内容）
"""
